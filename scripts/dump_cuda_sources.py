...
def cuda_output_path(py_file: Path, factory_name: str, multi_jit: bool) -> Path:
...
def format_header(
    ...
    lines = [
        ...
        f"// Compile args: {json.dumps(serialize_value(compile_args), sort_keys=True)}",
    ]
    if public_api:
...
def main() -> int:
    ...
    for py_file in kernel_modules:
        ...
        for factory_name, factory in factories:
            spec = module_specs.get(factory_name)
            if spec is None:
            ...
            try:
                ...
                failures.append(f"{import_path}::{factory_name}:\n{traceback.format_exc()}")
                continue

            out_path = cuda_output_path(py_file, factory_name, multi_jit)
    ...
    if len(manifest) != 48:
        print(f"WARN: expected 48 dumps, got {len(manifest)}", file=sys.stderr)
        return 1
